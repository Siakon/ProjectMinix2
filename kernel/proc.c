...
/* Changing the priority of a system process */
static void change_prio_head(struct proc *rp, int priority);
static void change_prio(struct proc *rp, int priority);
static int initialize_policy(void);
...
/* Set the flow of the policy */
int factor = 0;

/* The whitelist array with the endpoints of the processes */
int proc_whitelist[6] = {0,1,2,8,9,10}; 
 ...
 */
  ...
  /* If the process is a special selected process then handle it differently.
   * Search the queue and if this process has lower priority than the first 
   * process of this queue move it first, else "throw" it on the queue. */
    int listed_proc = 0;
	for(int i=0; i < 6; i++){
	...
	if(q == SPECIAL_QUEUE1 && policy_is_ok == 1 && listed_proc == 1){
		...
		if (!rdy_head[q]) {		/* add to empty queue */
		  ...
		  rp->p_nextready = NULL;		/* mark new end */
			...
			if((rp->dyn_period < rdy_head[q]->dyn_period) || rdy_head[q]->dyn_period == 0){
 ...
 */
  ...
  for (q=0; q < NR_SCHED_QUEUES; q++) {	
	...
	/* If the queue is SPECIAL_QUEUE1 or SPECIAL_QUEUE2 then we have to choose
	 * a process differently according to it's period */
	/*
	if(q == SPECIAL_QUEUE1){
		lowest_period = rp->dyn_period;
		for (xpp = get_cpu_var_ptr(rp->p_cpu, run_q_head[q]); *xpp;
				xpp = &(*xpp)->p_nextready) {
			int listed_proc = 0;
			if(policy_is_ok == 1){
			    for(int i=0; i < 6; i++){
				    if((*xpp)->p_endpoint == proc_whitelist[i]){
					    listed_proc = 1;
					    break;
				    }  
			    }
			...
			if(policy_is_ok == 1 && listed_proc == 1){
				period = (*xpp)->dyn_period;
				/* Choose the process with the lowest period 
				 * If alter the comparison then choose the 
				 * the process with the highest period*/
	...
	/* Call the function calc_period() to calculate the approximate period of this process */
	calc_period(rp);
	
	/* Recalculate and fix the priorities */
	initialize_policy();
	return rp;
  }
  return NULL;
...
void calc_period(struct proc *rp){
...
void change_prio_head(struct proc *rp, int priority){
	...
	RTS_HEAD_UNSET(rp, RTS_PREEMPTED);
	
}

void change_prio(struct proc *rp, int priority){
...
int initialize_policy(void){
	
	
	/* The whitelisted system for now. */
	int count_list_array = 0;
	int ready_period = 0;
	
	/* Variables and tools to search and define the new priorities
	 * on the system processes we selected */
	 register struct proc *rp;
	 static struct proc *oldrp = BEG_PROC_ADDR;
	
	/* First make a check to validate that all the whitelisted processes
	 * have been created, if no do nothing, if yes start the re-adjustment
	 * and initiate the new policy */
	...
	if(policy_is_ok == 0){
		 for(rp = oldrp; rp < END_PROC_ADDR; rp++){
			...
			/* Check if the process is on the whitelisted processes. If yes then
			 * we are going to increment a variable so we will be able to see later
			 * if all of the whitelisted processes have been created. */
			 
			for(int i=0; i < 6; i++){
				if(rp->p_endpoint == proc_whitelist[i]){
					if(rp->period_is_ready == 1) ready_period++;
					count_list_array++;
					break;
				}
			}
		...
		/* if the policy if ok we have to empty the scheduling queue No 2
		 * this is the queue that we are going to use for our special
		 * processes. The cleance is going to take place only once since
		 * these processes, as preemptible, they have fixed priorities. */
		 
		if(count_list_array == 6 && ready_period == 6){
			policy_is_ok = 1;
		}	
