...
int do_rt_sample(struct proc *caller, message *m_ptr);

/* The kernel call to initiate the policy 
 * This call notifies the policy when the booting 
 * is completed */
int do_init_policy(struct proc * caller, message *m_ptr);
