...
{
/* Show the kernel queues, and especially the special queue where the policy
 * places the whitelisted processes */
	...
	while(1){
		...
		for(rp = oldrp; rp < END_PROC_ADDR; rp++){
			...
			if (rp->p_priority == 2){
				if(rp->p_nextready == prev_rp){
					printf("%d. Proc: %s, Prio: %u, Period: %u\n",counterj,proc_name(rp->p_nr),rp->p_priority,rp->dyn_period);
